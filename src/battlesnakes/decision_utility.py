...
def path_distance_pq(p, q, occupied=None):
    if occupied is None:
        occupied = game.state["occupied_cells"][0]
...
def chasing_tail():
    ...
    result_other = chasing_other_tail(game.state["others"][0])
    game.state["chasing_tail"] = {
        "me": result_me,
        "other": result_other,
    }
    if len(result_me) != 0:
...
def pos_on_board(pos: typing.Tuple) -> bool:
    ...
    if y < 0:
        return False
    if x >= game.state["board"]["width"]:
        return False
    if y >= game.state["board"]["height"]:
        return False
    return True
...
def on_border(coord: typing.Tuple) -> bool:
    x,y = coord
    if x == 0 or x == game.state["board"]["width"]-1:
        return True
    if y == 0 or y == game.state["board"]["height"]-1:
        return True
    return False
