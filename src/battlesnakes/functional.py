...
def food1(moves):
    ...
    if len(food1) != 0:
        food_and_tail = [a for a in moves if a in food1]
        if len(food_and_tail) != 0:
            return food_and_tail
        food_tail_connect = [a for a in food1 if any([path_connected(a, p) for p in moves])]
...
def replace_tail_path(moves):
    ...
    if path_distance_pq(g.s.my_head, first_inflection_point) <= first_inflection_index:
        ...
        if len(tail_moves) != 0:
            g.decision_path.append(f"replace tail path - direct {first_inflection_point}")
            return tail_moves
    else:
        tail_moves = shortest_path_move(g.s.my_head, first_inflection_point)
        meander_moves = [a for a in moves if a not in tail_moves]
        if len(meander_moves) != 0:
            g.decision_path.append(f"replace tail path - meander {first_inflection_point}")
...
def chase_my_tail(moves):
    if path_distance_pq(g.s.other_head, g.s.my_tail) > path_distance_pq(g.s.my_head, g.s.my_tail):
        if distance_pq(g.s.my_head, g.s.my_tail) >= 1:
            ...
            moves = [a for a in tail_moves if a in moves]
            if len(moves) != 0:
                return cases([
                    food1,
                    food4(g.s.my_tail),
